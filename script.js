...
function renderTable(data, stats) { $('service-table').innerHTML = data.services.map((service, index) => { const item = stats[index]; const unit = item.actualTotal > 999 ? 'un.' : ''; return `<tr><td>${service.name}</td><td><div class="dual-bars"><div class="bar-row"><div class="bar-track"><div class="bar-fill plan" style="width:${Math.min(item.planAtDate, 100)}%"></div></div><span class="bar-value">${formatNumber(item.planAtDate)}%</span></div><div class="bar-row"><div class="bar-track"><div class="bar-fill actual" style="width:${Math.min(item.actualAtDate, 100)}%"></div></div><span class="bar-value">${formatNumber(item.actualAtDate)}%</span></div></div></td><td class="service-percent">${formatNumber(item.actualAtDate)}%</td><td class="accumulated">${formatNumber(item.actualTotal)} ${unit}</td><td><span class="badge ${item.status}">${statusLabel(item.status)}</span></td></tr>`; }).join(''); }
